...
mainMenu = NativeUI.CreateMenu("Main Menu", "~b~Made by IceyScripts")
...
function FirstItem(menu)
   local checkbox = NativeUI.CreateCheckboxItem("Connection Test", bool, "test server ping")
...
function FourthItem(menu) 
   local submenu = _menuPool:AddSubMenu(menu, "~b~Spawner") 
   local carItem = NativeUI.CreateItem("Spawn super cars", "Spawn car in a bugatti")
   carItem.Activated = function(sender, item)
        if item == carItem then
            spawnCar("adder")
            notify("spawned in an bugatti")
        end
   end
